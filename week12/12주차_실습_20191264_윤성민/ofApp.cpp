...
bool ofApp::readFile()
{
	...
	if (openFileResult.bSuccess) {
		...
		if (pos != string::npos && pos != 0 && filePath.substr(pos + 1) == "maz") {
			...
			//cout << graph[0][1]->link->idx << " " << graph[0][1]->link->idy << endl;
...
/*void ofApp::createGraph() {
	...
	for (int i = 0; i < HEIGHT; i++) {
		for (int j = 0; j < WIDTH; j++) {
			for (int k = 0; k < 4; k++) {
				np = (node)malloc(sizeof(struct vertex));
				if (input[2 * i + 2][2 * j + 1] == ' ') { //down
					np->idx = i + 1; np->idy = j;
				}
				else if (input[2 * i + 1][2 * j + 2] == ' ') { //right
					np->idx = i; np->idy = j + 1;
				}
				else if (input[2 * i + 1][2 * j] == ' ') { //left
					np->idx = i; np->idy = j - 1;
				}
				else if (input[2 * i][2 * j + 1] == ' ') { //up
					np->idx = i - 1; np->idy = j;
				}
				np->link = NULL;
				if (prev == NULL) {
					graph[i][j] = np; prev = np;
				}
				else
					prev->link = np;
