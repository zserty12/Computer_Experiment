...
void InitTetris(){
	...
	recRoot->lv = 0;
	recRoot->total_score = 0;
	for (i = 0; i < HEIGHT; i++)
		for (j = 0; j < WIDTH; j++)
			recRoot->recf[i][j] = field[i][j];
...
void NodeCreate(RecNode* node, RecNode *root) {
	node->lv = root->lv + 1;
	node->total_score = 0;
	for (int i = 0; i < HEIGHT; i++)
		for (int j = 0; j < WIDTH; j++)
			node->recf[i][j] = root->recf[i][j];
}

int recommend(RecNode *root){
	int max=0; // 미리 보이는 블럭의 추천 배치까지 고려했을 때 얻을 수 있는 최대 점수
	int rec_score = 0, nodeidx = 0;
	int Rotate, recR, recX, recY;
	...
	for (recR = 0; recR < Rotate; recR++) {
		for (recX = -2; recX < 13; recX++) {
			recY = 0;
			if (CheckToMove(root->recf, nextBlock[root->lv], recR, recY, recX)) {
				RecNode* childnode = (RecNode*)malloc(sizeof(RecNode));
				NodeCreate(childnode, root);
				while (CheckToMove(childnode->recf, nextBlock[root->lv], recR, recY+1, recX)) recY++;
				//childnode->total_score = root->total_score;
				childnode->total_score += AddBlockToField(childnode->recf, nextBlock[root->lv], recR, recY, recX);
				childnode->total_score += DeleteLine(childnode->recf);
				rec_score = childnode->total_score;
				root->c[nodeidx++] = childnode;

				if (childnode->lv < VISIBLE_BLOCKS - 1)
					rec_score += recommend(childnode);
				//else
					//rec_score = root->c[nodeidx]->total_score;
